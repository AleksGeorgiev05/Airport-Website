...
namespace PlaneProject.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult FlightInsert(Flights flightModel)
        {
            ...
            int hour_of_arrival = Convert.ToInt32(flightModel.TimeOfArrival.Value.ToString("HH"));
            int hour_of_flight = Convert.ToInt32(flightModel.TimeOfFlight.Value.ToString("HH"));
            ...
            if (flightModel.TimeOfFlight.Value.ToUniversalTime() > flightModel.TimeOfArrival.Value.ToUniversalTime()&&dontCheck==false)
            {
                ViewBag.Message = "Invalid time of flight and time of arrival!";
                return View("CreateFlight",flightModel);
            }
            if (flightModel.TimeOfFlight.Value.ToUniversalTime() > flightModel.TimeOfArrival.Value.ToUniversalTime())
            {
                Check = false;
            }

            int days = Math.Abs(Convert.ToInt32(flightModel.TimeOfArrival.Value.ToString("dd")) - Convert.ToInt32(flightModel.TimeOfFlight.Value.ToString("dd")));
            if (Check)
            {
                flightModel.Duration = days * 24 + Math.Abs(Convert.ToInt32(flightModel.TimeOfArrival.Value.ToString("HH")) + Convert.ToInt32(flightModel.TimeOfFlight.Value.ToString("HH")));
            }
            else
            {
